...
export const appRouter = router({
  ...
  subscription: router({
    ...
    getStatus: protectedProcedure.query(async ({ ctx }) => {
      ...
      return await getSubscriptionStatus(dbInstance.default, ctx.user.id);
    }),
    ...
    createCheckout: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        const dbInstance = await import("./db");
        return await subscriptionRouter.createCheckout(
          ctx.user.id,
          ctx.user.email,
          input.priceId,
          input.interval
    ...
    createPortal: protectedProcedure.mutation(async ({ ctx }) => {
      const dbInstance = await import("./db");
      return await subscriptionRouter.createPortal(ctx.user.id, dbInstance.default);
    }),

    // Cancel subscription
    cancel: protectedProcedure.mutation(async ({ ctx }) => {
      const dbInstance = await import("./db");
      return await subscriptionRouter.cancel(ctx.user.id, dbInstance.default);
    }),

    // Reactivate subscription
    reactivate: protectedProcedure.mutation(async ({ ctx }) => {
      const dbInstance = await import("./db");
      return await subscriptionRouter.reactivate(ctx.user.id, dbInstance.default);
    }),
  }),
